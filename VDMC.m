function [U, V, B, sortB, obj] = VDMC(X, d, k, w, lambda, U_ini, V_ini, max_iter)
...
for iter = 1: max_iter
    ...
    thef = 0.5*A_vector-w*var_vector;
    ...
    for dd = 1:d
        ...
        % solve U
        U{dd} = SolveU(X{dd}, V{dd}, U{dd}, w, B);
    end;
...
function U = SolveU(X, V, U, w, B)
[~, K] = size(U);

tp = (V'*V-2*w/K*(eye(K)-ones(K)/K))\V';
IX = find(B > 0);
